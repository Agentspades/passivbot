...
# AI optimization imports (optional)
try:
    from ai_optimizer import AITrainer, setup_gpu
    AI_OPTIMIZATION_AVAILABLE = True
except ImportError:
    AI_OPTIMIZATION_AVAILABLE = False
    logging.warning("AI optimization modules not available. Install PyTorch dependencies for AI features.")
...
def add_extra_options(parser):
    ...
    parser.add_argument(
        "--ai-mode",
        action="store_true",
        help="Use AI-powered optimization instead of evolutionary algorithm",
    )
    parser.add_argument(
        "--ai-episodes",
        type=int,
        default=1000,
        help="Number of AI training episodes (default: 1000)",
    )
    parser.add_argument(
        "--ai-model-path",
        type=str,
        default=None,
        help="Path to pre-trained AI model to load",
    )
    parser.add_argument(
        "--hybrid-mode",
        action="store_true",
        help="Use hybrid AI + evolutionary optimization",
    )
...
def run_ai_optimization(config, args, bounds, sig_digits, results_queue):
    """Run pure AI-powered optimization."""
    if not AI_OPTIMIZATION_AVAILABLE:
        raise ImportError("AI optimization modules not available")
    
    logging.info("Initializing AI trainer...")
    
    # Setup AI trainer
    trainer = AITrainer(
        config_path=args.config_path or "configs/template.json",
    ...
    if args.ai_model_path:
        ...
        logging.info(f"Loaded pre-trained model from {args.ai_model_path}")
    ...
    logging.info(f"Best performance: {training_summary['performance_metrics']['best_global_performance']}")
    
    # Convert AI results to population format for compatibility
    best_config = training_summary['performance_metrics']['best_global_config']
    if best_config:
        population = [creator.Individual(best_config)]
    else:
        population = []
    
    # Create dummy logbook for compatibility
...
def run_hybrid_optimization(config, args, bounds, sig_digits, population, toolbox, stats, hof, results_queue):
    """Run hybrid AI + evolutionary optimization."""
    if not AI_OPTIMIZATION_AVAILABLE:
        raise ImportError("AI optimization modules not available")
    
    logging.info("Initializing hybrid optimization...")
    ...
    # Phase 1: AI pre-training (reduced episodes for hybrid mode)
    ai_episodes = max(100, args.ai_episodes // 5)
    logging.info(f"Phase 1: AI pre-training with {ai_episodes} episodes...")
    
    training_summary = trainer.train(
        num_episodes=ai_episodes,
        max_episode_steps=30,
        update_frequency=5,
        save_frequency=50
    )
    
    # Phase 2: Use AI suggestions to seed evolutionary algorithm
    logging.info("Phase 2: AI-guided evolutionary optimization...")
    
    # Get AI suggestions for promising parameter regions
    ai_suggestions = get_ai_suggestions(trainer, bounds, num_suggestions=len(population) // 4)
    
    # Replace some population members with AI suggestions
    for i, suggestion in enumerate(ai_suggestions):
        if i < len(population):
            population[i] = creator.Individual(suggestion)
    
    # Phase 3: Run evolutionary algorithm with AI-seeded population
    reduced_generations = max(1, int(config["optimize"]["iters"] / len(population)) // 2)
    ...
    population, logbook = algorithms.eaMuPlusLambda(
        ...
        ngen=reduced_generations,
        stats=stats,
        halloffame=hof,
        verbose=False,
    )
    
    logging.info("Hybrid optimization completed!")
    
    return population, logbook


def get_ai_suggestions(trainer, bounds, num_suggestions=10):
    """Get parameter suggestions from trained AI agent."""
    suggestions = []
    
    for _ in range(num_suggestions):
        # Evaluate the agent to get a suggestion
        eval_result = trainer.evaluate(num_episodes=1)
        
        if eval_result['episode_performances']:
            # Get the configuration that produced the best performance
            best_performance = eval_result['episode_performances'][0]
            
            # Convert performance back to individual (this is simplified)
            # In practice, you'd want to track the actual configurations
            suggestion = []
            for low, high in bounds:
                if high == low:
                    suggestion.append(low)
                else:
                    # Generate based on AI learned patterns (simplified)
...
async def main():
    ...
    try:
        ...
        if args.ai_mode and AI_OPTIMIZATION_AVAILABLE:
            logging.info(f"Starting AI-powered optimization...")
            population, logbook = run_ai_optimization(
                config, args, bounds, sig_digits, results_queue
            )
        elif args.hybrid_mode and AI_OPTIMIZATION_AVAILABLE:
            logging.info(f"Starting hybrid AI + evolutionary optimization...")
            population, logbook = run_hybrid_optimization(
                config, args, bounds, sig_digits, population, toolbox, stats, hof, results_queue
            )
        else:
            logging.info(f"Starting evolutionary optimization...")
            population, logbook = algorithms.eaMuPlusLambda(
                population,
                toolbox,
                mu=config["optimize"]["population_size"],
